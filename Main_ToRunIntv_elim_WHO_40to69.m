...
%          WormKill = 0.35 + 0.0*rand(length(mfPrevArray(1,:)),1);  % for MDA
% %         
...
%                  WormKill = 0.55+ 0.0*rand(length(mfPrevArray(1,:)),1);  % for IDA
        ...
        tWHO = 49;
            ...
            tWHO2 = Time_toCross_below_Threshold_mean(mfPrevIntv1,EP_Th_ABR(iReg,3));
            
            tWHO1 = Time_toCross_below_Threshold(mfPrevIntv1,EP_Th_ABR(iReg,3)); % finds month where 1% is reached
             NumYears = (((tWHO-1)/12)+20); % change number of years to simulation
            MonthlyMDACov(tWHO:NumYears*12) = 0; % change MDA cov to zero after crossing threshold
            ITNCoverages(tWHO:NumYears*12) = 0; % change ITN cov to zero after crossing threshold
            IRSCoverages(tWHO:NumYears*12) = 0;
            MDAInterval(tWHO:NumYears*12) = 0;
...
%              mfPrevIntv6to7(:,:) = 100*mfAgeprevFun(sumMf(cVec_p(5*12+1:7*12,:),ageMthMax_g,da_g,demo_g),...
%                 negBinshapeFun(sumMf(cVec_p(5*12+1:7*12,:),ageMthMax_g,da_g,demo_g),k0,kLin));
...
%             %     p=find(mfPrevIntv3(tWHO1,:)<=EP_Th_ABR(iReg));
            for j = 1:length(mfPrevIntv3(1,:))
                if (mfPrevIntv3((((tWHO-1)/12)+20)*12,j)>mfPrevIntv3(tWHO,j) ) 
                    id5 = [id5,j];
                    count1 = count1 + 1;
                else
                    id6 = [id6,j];
    ...
    prec6 = round(length(id7)/(length([id7,id8]))*100);
           ...
           save(sprintf('Intv_WHO_MDA_40to69_%s.mat',char(Sites{iSites})),...
                'mfPrevIntv1','MBRIntv1','L3Intv1','RegimenEfficacy',...
                'MonthlyMDACov','IRSCoverages',...
                'ITNCoverages','MfData','SwitchMonth','mfPrevIntv3',...
                'MBRIntv3','L3Intv3','tWHO1','WormKill','prec1',...
                'prec2','prec3','prec4','prec5','prec6','id1','id2',...
                'id3','id4','id5','id6','id7','id8','tWHO','tWHO2');
        end
