...
u1=mfPrevIntv3((((tWHO-1)/12)+5)*12,re1(1:end));
v1=mfPrevIntv3(35*12,re1(1:end));
...
elimination5 = round(length(e)/(length([eli1,eli2]))*100);
stick5 = round(length(f)/(length([eli1,eli2]))*100);
u2=mfPrevIntv3((((tWHO-1)/12)+5)*12,re1(1:end));
v2=mfPrevIntv3(35*12,re1(1:end));


load(sprintf('Intv_EP_MDA_6to7_%s.mat',Sites{iSites}));
        count = 0; % count number of curves that rise back to 1%
            re1 = []; % curves that rise back to 1%
            re2 = []; % curves that stay below 1%
            for j = 1:length(mfPrevIntv3(1,:))
...
subplot(2,2,3)
count = 0; % count number of curves that rise back to 1%
            f1 = []; % curves that rise back to 1%
            f2 = []; % curves that stay below 1%
            
            for j = 1:length(mfPrevIntv3(1,:))
                if (mfPrevIntv3(35*12,j)<0.5)
                    f1 = [f1,j];
                    count = count + 1;
                else
                    f2 = [f2,j];
...
if (f1>0)
semilogy((1:length(mfPrevIntv3(:,1)))/12,(mfPrevIntv3(:,f1)),'Color',[0.6;0.6;0.6]);
hold on;
end
hold on;
% load('Intv_WHO_MDA_DokanTofa.mat')
...
for i=1:length(re1)
    for j=tWHO+1:421
        if(mfPrevIntv3(j-1,re1(i))<mfPrevIntv3(j,re1(i)) &&  mfPrevIntv3(j-1,re1(i))<mfPrevIntv3(421,re1(i)) &&  mfPrevIntv3(421,re1(i))<0.5)
            id10(i)=j-1;
           break;
        end
    end
...
plot([0 35],[EP_Th_ABR(iReg,3) EP_Th_ABR(iReg,3)],'-k');
hold on;
plot([((tWHO-1)/12) ((tWHO-1)/12)],[10^-14 10^2],'--b');
hold on;
plot([((tWHO-1)/12)+5 ((tWHO-1)/12)+5],[10^-14 10^2],'-b');
hold on;
set(gca,'XLim',[0 35]);
...
elimination5 = round(length(e)/(length([eli1,eli2]))*100);
stick5 = round(length(f)/(length([eli1,eli2]))*100);
u3=mfPrevIntv3((((tWHO-1)/12)+5)*12,re1(a));
v3=mfPrevIntv3(35*12,re1(a));



 load(sprintf('Intv_EP_MDA_40to69_%s.mat',Sites{iSites}));
        count = 0; % count number of curves that rise back to 1%
            re1 = []; % curves that rise back to 1%
            re2 = []; % curves that stay below 1%
            for j = 1:length(mfPrevIntv3(1,:))
...
u4=mfPrevIntv3((((tWHO-1)/12)+5)*12,re1(a));
v4=mfPrevIntv3(35*12,re1(a));
    end
end
 save(sprintf('mfprev_6to7_40to70_DokanTofa.mat'),...
